> **⚠ ARQUIVO — NÃO USAR COMO LISTA DE TRABALHO**
>
> Este documento descreve o estado do código em **v1.8.13 (2026-08-18)**. Foi auditado em 2026-08-20 (v1.13.0) e constatou-se que a maioria dos claims já não corresponde à realidade. Veja a tabela de claims refutados abaixo antes de agir sobre qualquer item.
>
> O trabalho activo é rastreado em **#352** (GitHub Issues). Não usar este ficheiro como lista de tarefas.
>
> ---
>
> **Claims verificados como falsos em 2026-08-20 (auditoria sobre v1.13.0):**
>
> | Secção | Claim original | Realidade actual |
> |---|---|---|
> | §1 | Media controls não declarados, UI usa `(mod as any)` | Declarados em `index.ts:196-198`, implementados via `MediaSessionManager`. Zero `as any` em `src/` |
> | §3 | MapsScreen delega via `Linking.openURL("geo:...")` | Sem `geo:` no ficheiro; conclusão (sem mapa real) mantém-se, mecanismo descrito não |
> | §5 | 42 `console.warn` removidos pelo `transform-remove-console` | `babel.config.js:7` exclui `error` e `warn`; `catch` usam `console.error` |
> | §6 | Toggle VPN cosmético é risco de segurança | Toggle já não existe em `VpnScreen.tsx` |
> | §7 | SoftwareUpdateScreen sempre "up to date" | Faz verificação real via API de Releases do GitHub + `semverGt` |
> | N1 | Faltam permissões `READ_PHONE_NUMBERS`, `ACCESS_NETWORK_STATE`, `ACCESS_COARSE_LOCATION`, BT scan | Todas declaradas em `app.json` |
> | N4 | EditProfileScreen sem validação de email | Valida com regex e bloqueia o save |
> | N5 | Ainda há `Alert.alert()` nativo | Zero em todos os ecrãs; todos usam `useAlert()` |
> | N6 | 14 ecrãs com `navigation: any` | Zero; só sobram comentários `eslint-disable` inertes |
>
> **Claim pior do que descrito:** `DELEGATION §3` diz que HotspotScreen tem leitura in-app a funcionar — não tem leitura nenhuma (ver #356).
