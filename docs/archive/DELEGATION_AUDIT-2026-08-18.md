...
>
> Este documento descreve o estado das delegações em **2026-08-18 (v1.8.x → v1.9.x)**. Foi auditado em 2026-08-20 (v1.13.0) e a maioria das delegações descritas como pendentes já foi resolvida.
>
> O trabalho activo é rastreado em **#352** (GitHub Issues). Não usar este ficheiro como lista de tarefas.
>
> ---
>
> **Claims verificados como falsos em 2026-08-20:**
>
> | Item | Claim original | Realidade actual |
> |---|---|---|
> | 🟡#1 | Join Wi-Fi exige painel do sistema no Android 10+ | `joinWifiNetwork` usa `WifiNetworkSuggestion` in-app (`LauncherModule.kt:190-211`) |
> | 🟡#2 | Pairing de Bluetooth exige painel do sistema | `createBond()` in-app (`LauncherModule.kt:339-348`); só o diálogo de PIN do OS é inevitável |
> | 🔴#12 | `tel:`/`sms:` devem passar a `makeCall()`/navegação in-app | Já passaram; `Linking.openURL` restante é só fallback |
> | 🟢#14,#15,#17 | Focus/Airplane no Control Center e Settings delegam | In-app e persistidos no `SettingsStore` |
> | 🟢#1-#13 | 17 delegações a remover | Todas removidas; o único escape em `src/screens/settings/` é `KeyboardScreen.tsx:149` (avariado — ver #361) |
>
> **Claim pior do que descrito:** `🟡#3` diz que HotspotScreen tem leitura in-app a funcionar — não tem leitura nenhuma (ver #356).
