 ...
 *
 * FICHEIRO PROPRIO, e nao um bloco no index.test.ts, por uma razao mecanica: este
 * ficheiro precisa de `jest.mock('expo')` a devolver o SEU fixture nativo e de
 * `jest.mock('react-native')` para poder virar Platform.OS entre describes. O
 * index.test.ts ja tem um `jest.mock('expo')` proprio e nao mocka o react-native
 * — e so pode haver um por ficheiro. A versao anterior deste PR resolveu isso
 * substituindo o index.test.ts inteiro, o que apagou os 32 testes que la estavam
 * (listeners de notificacoes #196, tecla HOME #508, deduplicacao, reporte de erros
 * da ponte), todos a passar em main. Em ficheiros separados, os dois conjuntos
 * coexistem com os mocks que cada um precisa.
 *
