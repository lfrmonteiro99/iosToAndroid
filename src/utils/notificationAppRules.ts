...
export function routeNotification(
  ...
  const policy = ctx.perAppDelivery?.[pkg ?? ''];
...
/** Rótulo legível da política de uma app (ausência = immediate). */
export function policyLabelFor(delivery: PerAppDelivery | null | undefined, packageName: string): string {
  const policy = delivery?.[packageName] ?? 'immediate';
  return APP_DELIVERY_LABELS[policy];
}
